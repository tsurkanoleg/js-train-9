...
function findElementGreaterThan(arr, num) {
	...
	const a = arr.find(elem => elem > num)
...
function findIndexByProperty(arr, name) {
	...
	const a = arr.findIndex(elem => elem.name === name);
...
function findLastEvenNumber(arr) {
	...
	const a = arr.findLast(elem => elem % 2 === 0)
	if (a !== undefined) {
		return a;
	} else {
...
function getPositiveArrayValues(arr) {
	...
	const positivArrayValues = arrayValues.filter((values) => values > 0)
	return positivArrayValues;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function sumAndPrint(arr) {
	...
	const a = arr.reduce((accumulator, currentValue) => accumulator + currentValue);
...
function flattenSquareAndFilter(arr, threshold) {
	...
	const a = arr.flat()
	const b = a.map (x => x ** 2);
	const c = b.filter(b => b > threshold);
	return c;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
	...
	const a = arr.slice(-n) ;
	const b = a.reduceRight(((accumulator, currentValue) => accumulator + currentValue));
...
function findLongestWord(arr) {
	...
	const longestWord = arr.reduce((elem1, elem2) => {
...
function calculateTotalPrice(arr) {
	...
	const totalPrice = arr.reduce((accumulator, currentItem) => {
		if (typeof currentItem === 'object' && 'price' in currentItem) {
